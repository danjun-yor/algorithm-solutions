...
rl.on("line", function(line) {
  ...
  // 첫줄: 테스트케이스 수
  // 각 테스트케이스
  //   첫줄: N(문서의 수) M(몇번째로 인쇄됐는지 궁금한 문서의 Queue에서의 위치)
  //   둘째줄: N개 문서에 대한 각각의 중요도

  // Queue 맨앞에 중요도보다 높은 문서가 뒤에 있을 때, 해당 문서를 맨 뒤로 보낸다.
  // 중요도에 따라 인쇄했을 때, M이 몇번째로 인쇄되었는가
  ...
  for (let i = 0; i < T; i++) {
    ...
    const queue = lines
      ...
      .split(" ")
      .map((v, i) => {
        return { value: parseInt(v), index: i };
      });

    let order = 0;
    const checkDoc = queue[m]; // 체크해야할 문서
    while (queue.length > 0) {
      const hasBigger =
        queue.findIndex((doc, i) => doc.value > queue[0].value) >= 0
          ? true
          : false;
      //중요도가 더 높은게 있으면
      if (hasBigger) {
        ...
        const printedDoc = queue.shift();
        order++;
        // 현재 출력된 문서가 알고싶은 문서이면
        if (checkDoc.index === printedDoc.index) {
          // 순서를 출력
          console.log(order);
          break;
        }

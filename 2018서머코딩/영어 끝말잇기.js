function solution(n = 2, words = []) {
  // n: 참여한 사람의 수
  // words: 끝말잇기에 사용된 단어들이 "순서대로" 들어있는 배열
  // 같은 단어를 또 사용하면 탈락
  // 다른 문자로 끝말은 이으면 탈락
  // 리턴: [가장 먼저 탈락하는 사람의 번호, 자신의 몇 번째 차례에 탈락하는지]
  // 탈락자가 생기지 않는다면: [0, 0] 리턴
  // 모든 단어는 소문자

  // 중복된 단어가 있는지 찾자
  for (let i = 1; i < words.length; i++) {
    // 내가 말한 단어 앞에 중복 단어가 있으면
    if (words.indexOf(words[i]) < i) {
      return [(i % n) + 1, Math.ceil((i + 1) / n)];
    }
    // 끝말잇기를 틀렸으면
    if (words[i - 1].split("").pop() !== words[i][0]) {
      return [(i % n) + 1, Math.ceil((i + 1) / n)];
    }
  }
  return [0, 0];
...
(계속 진행)
끝말잇기를 계속 진행해 나가다 보면, 
3번 사람이 자신의 세 번째 차례에 말한 
tank 라는 단어는 이전에 등장했던 단어이므로 탈락하게 됩니다.

사람의 수 n과 사람들이 순서대로 말한 단어 words 가 매개변수로 주어질 때, 
가장 먼저 탈락하는 사람의 번호와 
그 사람이 자신의 몇 번째 차례에 탈락하는지를 구해서 
